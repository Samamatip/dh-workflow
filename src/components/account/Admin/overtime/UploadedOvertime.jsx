...
const UploadedOvertime = ({ shifts, selectedYearMonth, setSelectedYearMonth, loading }) => {
  ...
  return (
    <div className='text-text-gray'>
      <div className="flex lg:flex-row flex-col gap-5 py-5 w-full">
        <div className="w-full lg:w-1/6 gap-1 flex flex-col">
          {/* upload shifts */}
          <button
            onClick={() => (window.location.href = '/overtime/upload-shifts')}
            title="Upload Shifts"
            className="w-full mb-5 hover:scale-105 transition-transform duration-200 bg-secondary text-white py-2 rounded-lg shadow-md flex items-center justify-center hover:bg-secondary-dark cursor-pointer"
          >
            Upload Shifts
          </button>

          {/* Render department names and number of shifts */}
          <ul className="px-1 py-2 rounded-lg shadow-md lg:max-h-fit max-h-28 overflow-y-auto scrollbar-thin bg-primary">
            <h4 className="text-white w-full border-b text-center font-semibold mb-2">Total Published shifts</h4>
            {shifts &&
              shifts.map((dept) => (
                <li key={dept.department._id}>
                  ...
                  <span className="ml-2 text-white">: {dept?.shift?.reduce((total, shift) => total + shift.quantity, 0)} Shifts</span>
                </li>
              ))}
          </ul>
        </div>
        ...
        <div className="lg:w-2/3 w-full sticky top-48 z-7 overflow-hidden">
          ...
          <h2 className="bg-background-1 p-2 w-full flex flex-col sm:flex-row items-start sm:items-center gap-2 justify-between">
            <span onClick={() => (window.location.href = '/overtime/set-rules')} title="Set Overtime rules" className="text-error font-semibold hover:underline cursor-pointer">
              Set Overtime rules
            </span>
            <span>
              <label htmlFor="monthYearSelect" className="text-primary font-semibold">
          ...
          {loading ? (
            <div className="text-center p-8">
              ...
              <p className="text-gray-600 text-lg">Loading published shifts...</p>
            </div>
          ) : shifts &&
            shifts.map((dept) => (
              <div key={dept.department._id} className="mb-5 bg-primary rounded-sm p-2">
                ...
                {openShifts[dept.department._id] && (
                  ...
                  <ul className="p-1 bg-gray-100 rounded-sm max-h-[50vh] overflow-y-auto scrollbar-thin">
                    {loading ? (
                      ...
                      <li className="text-text-gray text-sm p-5">No shifts published for this department in this period</li>
                    )}
                  </ul>
                )}
              </div>
...
export default UploadedOvertime;
