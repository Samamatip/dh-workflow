...
import { getAvailablePublishedShiftsByOtherDepartmentService, bookShiftService } from '@/services/shiftsServices';
import { useUndevelopedFunctionality } from '@/contexts/UndevelopedFunctionalityWarning';
import React from 'react';

const AvailableOvertimeInOtherLocation = ({selectedYearMonth, setSelectedYearMonth}) => {
  ...
  React.useEffect(() => {
    const fetchShifts = async () => {
      ...
      setLoading(true);
      try {
        const response = await getAvailablePublishedShiftsByOtherDepartmentService(user.id, selectedYearMonth);
        if (response.data) {
  ...
  return (
    <div className='flex flex-col w-full'>
        ...
        <div className="p-4">
          <h3 className='text-primary font-semibold mb-4'>Available Shifts in Other Departments</h3>
          
          {loading && <p>Loading...</p>}
          {message && <p className={message.includes('Error') || message.includes('Failed') ? 'text-red-600' : 'text-green-600'}>{message}</p>}
          
          {shifts.length === 0 && !loading && (
            <p className="text-gray-500">No available shifts in other departments for this month.</p>
          )}
                  ...
                  <div className="flex-1">
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      <div>
                        ...
                        <p className="font-semibold text-secondary">{shift.department?.name}</p>
                      </div>
                      <div>
...
export default AvailableOvertimeInOtherLocation;